...
// анти-дубль: защита от повторных запросов
...
// Лимиты
const SEC_LIMIT = 2;   // макс. 2 спина в секунду
const MIN_LIMIT = 60;  // макс. 60 в минуту
...
export const spinSlot = async (req, res) => {
  ...
  try {
    ...
    // 🔒 анти-дубль + лок + rate-limit
    if (!dedupeOnce(`slot:${telegram_id}`)) {
      return res.status(409).json({ error: "Duplicate spin request" });
    }
    ...
    try {
      ...
      return res.json({
        ...
        prize:
          isStarsLike
            ? { type: prize_type, amount: computedPrize }
