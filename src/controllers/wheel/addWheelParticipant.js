...
// ---- lightweight locks (в рамках одного процесса) ----
const RUN_USERS = new Set();
const RUN_WHEELS = new Set();
const sleep = (ms)=>new Promise(r=>setTimeout(r,ms));
async function acquire(set,key){ while(set.has(key)) await sleep(15); set.add(key); }
function release(set,key){ set.delete(key); }
...
export const joinWheel = async (req, res) => {
  ...
  // 🔒 не даём параллельно вступать тем же юзером или в то же колесо
  await acquire(RUN_USERS, telegram_id);
  await acquire(RUN_WHEELS, wheel_id);
  try {
    ...
    // 5) Кол-во участников и заполненность (первая проверка)
    ...
    // 6) Списываем билеты атомарно в БД (если цена > 0)
    if (Number(wheel.price) > 0) {
      const { data: debited, error: debitErr } = await supabase.rpc(
        'debit_tickets_if_enough',
        { p_user: user_id, p_amount: Number(wheel.price) }
      );
      if (debitErr) return res.status(500).json({ error: 'Debit failed' });
      const ok = Array.isArray(debited) && debited.length > 0; // row returned => хватило
      if (!ok) return res.status(400).json({ error: 'Not enough tickets to join this wheel' });
    }

    // 7) Повторная проверка заполненности прямо перед вставкой
    const { count: cnt2, error: cnt2Err } = await supabase
      .from('wheel_participants')
      .select('*', { count: 'exact', head: true })
      .eq('wheel_id', wheel_id);
    if (cnt2Err) return res.status(500).json({ error: 'Failed to count participants (final)' });
    if (cnt2 >= wheel.size) return res.status(403).json({ error: 'Wheel is full' });

    // 8) Добавляем участника (уникальный индекс защитит от дублей)
    ...
    // 10) Автозапуск, если колесо заполнилось
    ...
    release(RUN_WHEELS, wheel_id);
    release(RUN_USERS, telegram_id);
  }
};
